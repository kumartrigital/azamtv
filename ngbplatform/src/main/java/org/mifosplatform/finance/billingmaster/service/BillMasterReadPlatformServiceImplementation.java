...
public class BillMasterReadPlatformServiceImplementation implements BillMasterReadPlatformService {
	...
	@Override
	public Page<FinancialTransactionsData> retrieveInvoiceFinancialData(final SearchSqlQuery searchTransactionHistory,
			final Long clientId) {
		...
		if (sqlSearch != null) {
			...
			extraCriteria = " and (v.transType like '%" + sqlSearch + "%' OR "
					+ " DATE_FORMAT(v.transDate,'%d %M %Y') like '%" + sqlSearch + "%' OR v.tran_type like '%"
					+ sqlSearch + "%' OR " + " v.dr_amt like '%" + sqlSearch + "%' OR v.cr_amt like '%" + sqlSearch
					+ "%' )";
		}
	...
	public Page<FinancialTransactionsData> retrieveTransactionalDatawithCurrency(final SearchSqlQuery searchTransactionHistory,
		...
		StringBuilder sqlBuilder = new StringBuilder(200);
		sqlBuilder.append("select ");
		sqlBuilder.append(financialTransactionsMapper.financialTransactionsSchemawithCurrency());
		String sqlSearch = searchTransactionHistory.getSqlSearch();
		String extraCriteria = "";
		if (sqlSearch != null) {
			sqlSearch = sqlSearch.trim();
	...
	private static final class FinancialInvoiceTransactionsMapper implements RowMapper<FinancialTransactionsData> {
		...
		public FinancialTransactionsData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			return new FinancialTransactionsData(null, null, transactionId, transDate, transactionType, debitAmount,
	...
	@Override
	public Page<FinancialTransactionsData> retrieveInvoiceFinancialDataByOfficeId(
			final SearchSqlQuery searchTransactionHistory, final Long officeId) {
	...
	private static final class FinancialInvoiceTransactionsMapperForOfficeData
			implements RowMapper<FinancialTransactionsData> {
		...
		public FinancialTransactionsData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			final LocalDate transDate = JdbcSupport.getLocalDate(resultSet, "TransDate");
			final String currency = resultSet.getString("currency");
			return new FinancialTransactionsData(null, clientId, transactionId, transDate, transactionType, debitAmount,
					creditAmount, null, userName, transactionCategory, flag, null, null, currency);
		}
	...
	public Page<BillDetailsData> retrieveStatments(SearchSqlQuery searchCodes, Long clientId) {
		...
		if (sqlSearch != null) {
			sqlSearch = sqlSearch.trim();
			extraCriteria = "  and (b.Due_amount like '%" + sqlSearch
					+ "%' or DATE_FORMAT(b.bill_date,'%d %M %Y') like '%" + sqlSearch + "%' "
	...
	private static final class InvoiceDataMapper implements RowMapper<FinancialTransactionsData> {
		...
		public FinancialTransactionsData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			// final String currency= resultSet.getString("currency");
			return new FinancialTransactionsData(chargeId, chargeType, chargeDescription, chargeAmount, taxAmount,
					discountAmount, netChargeAmount, chargeStartDate, chargeEndDate, orderId);
		}
	...
	@Override
	public Page<FinancialTransactionsData> retrieveSampleData(final SearchSqlQuery searchTransactionHistory,
			final Long clientId, final String type) {
	...
	private static final class FinancialTypeMapper implements RowMapper<FinancialTransactionsData> {
		...
		public FinancialTransactionsData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			/*
			 * FinancialTransactionsData ft = new FinancialTransactionsData(null,
			 * transactionId, transDate, transactionType, debitAmount, creditAmount, null,
	...
	@Override
	public Page<FinancialTransactionsData> retrieveStatementsData(SearchSqlQuery searchFinancialTransactionNew,
			Long clientId, String type, String fromDate, String toDate) {
		...
		if (fromDate != null && toDate != null) {
			sqlBuilder.append(
					" and date_format(bp.payment_date,'%Y-%m-%d') between '" + fromDate + "' and '" + toDate + "' ");
		}
		sqlBuilder.append(" ) a ");
		sqlBuilder.append(" WHERE a.client_id =" + clientId);
		/*
		 * if(fromDate != null && toDate != null && type != null) {
		 * sqlBuilder.append(" and tckt.status='"
		 * +type+"' and DATE_FORMAT(bp.payment_date,'%Y-%m-%d') between '"
		 * +fromDate+"' and '"+toDate+"' "); }
		 */
	...
	@Override
	public Page<FinancialTransactionsData> retrieveStatementsDatawithCurrency(
			SearchSqlQuery searchFinancialTransactionNew, Long clientId, Long currencyId, String type, String fromDate,
			String toDate) {
	...
	private static final class FinancialTypeNewMapper implements RowMapper<FinancialTransactionsData> {
		...
		public FinancialTransactionsData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			FinancialTransactionsData ft = new FinancialTransactionsData(null, null, transactionId, transDate,
					transactionType, debitAmount, creditAmount, null, userName, transactionCategory, flag, null, null,
					currency);
	...
	@Override
	public List<FinancialTransactionsData> retriveDataForDownload(final Long clientId, final String startDate,
			final String endDate) {

		StringBuilder builder = new StringBuilder(200);
		builder.append(
				"select ft.username as username,ft.transId as TransId,ft.transType as TransType,ft.Dr_amt as DebitAmount,ft.Cr_amt as CreditAmount,"
						+ "ft.tran_type as tran_type,ft.flag as flag,ft.transDate as TransDate from fin_trans_vw as ft where client_id="
						+ clientId);

		builder.append(" and date_format(transDate,'%Y-%m-%d') between '" + startDate + "' and '" + endDate
	...
	public List<BillDetailsData> retrieveStatements(SearchSqlQuery searchCodes, Long clientId) {
		...
		sqlBuilder.append(sql);
		/*
		 * String sqlSearch=searchCodes.getSqlSearch(); String extraCriteria = ""; if
		 * (sqlSearch != null) { sqlSearch=sqlSearch.trim(); extraCriteria =
		 * "  and (b.Due_amount like '%"
		 * +sqlSearch+"%' or DATE_FORMAT(b.bill_date,'%d %M %Y') like '%"
		 * +sqlSearch+"%' " +
		 * " or DATE_FORMAT(b.due_date,'%d %M %Y') like '%"+sqlSearch+"%' )"; }
		 * 
		 * sqlBuilder.append(extraCriteria);
		...
		/*
		 * if (searchCodes.isLimited()) {
		 * sqlBuilder.append(" limit ").append(searchCodes.getLimit()); } if
		 * (searchCodes.isOffset()) {
		 * sqlBuilder.append(" offset ").append(searchCodes.getOffset()); }
		 */
